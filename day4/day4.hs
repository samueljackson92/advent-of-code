...
main = do
	args <- getArgs
	putStrLn . show . findHash $ (args !! 0)


findHash :: String -> Int
findHash x = incrementHash 0 x
	where
		incrementHash :: Int -> String -> Int 
		incrementHash num s
			| checkHash (makeHash num s)  =  num
			| otherwise   = incrementHash (num+1) s
		
		checkHash :: String -> Bool
		checkHash ('0':'0':'0':'0':'0':'0':a) = True
		checkHash _ = False 

makeHash :: Int -> String -> String
makeHash num s = show . md5 . C.pack $ (s ++ (show num))
	where 
